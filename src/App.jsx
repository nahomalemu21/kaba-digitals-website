...
export default function App() {
  ...
  return <main className="min-h-screen text-[#f7f3e8] overflow-x-hidden font-[Inter,system-ui,sans-serif]" style={{background:'#050505'}}>
    {showIntro && <div style={{position:'fixed',inset:0,zIndex:999,background:'#050505',display:'grid',placeItems:'center',overflow:'hidden'}}>
      <div style={{position:'absolute',inset:0,background:'radial-gradient(circle at 50% 30%,rgba(229,185,87,.18),transparent 28%)',opacity:.75}}/>
      <div style={{position:'relative',textAlign:'center'}}>
        <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:18,marginBottom:18}}>
          {['K','A','B','A'].map((l,i) => <span key={i} style={{fontSize:'clamp(72px,12vw,180px)',fontWeight:950,lineHeight:.8,letterSpacing:'-.08em',color:'#f7f3e8',animation:`letterDrop .8s cubic-bezier(.2,.8,.2,1) ${i*.15}s both`}}>{l}</span>)}
        </div>
        <strong style={{display:'inline-block',color:'#e5b957',letterSpacing:'.42em',fontSize:18,marginLeft:'.42em',animation:'fadeUp .7s ease .9s both'}}>LABS</strong>
        <p style={{color:'#a7a096',fontWeight:700,margin:'18px 0 24px',animation:'fadeUp .7s ease 1.15s both'}}>Growth Systems for Ethiopian Businesses</p>
        <button onClick={() => setShowIntro(false)} style={{border:'1px solid rgba(229,185,87,.35)',background:'linear-gradient(135deg,#e5b957,#fff0a8)',color:'#050505',borderRadius:999,padding:'14px 18px',fontWeight:900,display:'inline-flex',alignItems:'center',gap:8,cursor:'pointer',animation:'fadeUp .7s ease 1.35s both'}}>Enter the Lab →</button>
      </div>
      <style>{`@keyframes letterDrop{from{opacity:0;transform:translateY(46px) scale(.9);filter:blur(10px)}to{opacity:1;transform:translateY(0) scale(1);filter:blur(0)}}@keyframes fadeUp{from{opacity:0;transform:translateY(16px)}to{opacity:1;transform:translateY(0)}}`}</style>
    </div>}
    <style>{`
    ...
    <header className="nav">
      ...
      <div className="lang">{['en','am','fr'].map(l => <button key={l} onClick={() => setLang(l)} className={lang===l?'on':''}>{l === 'am' ? 'አማ' : l.toUpperCase()}</button>)}</div>
    </header>
